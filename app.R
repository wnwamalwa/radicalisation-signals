...
server <- function(input, output, session) {
  ...
  output$dash_priority <- renderDT({
    ...
    d <- d[d$risk_level=="HIGH",]; d <- d[order(-d$risk_score),][1:min(8,nrow(d)),]
    d$nb <- sapply(as.integer(d$ncic_level), function(x) ncic_badge_html(x %||% 0L))
    d$rb <- sapply(as.character(d$risk_level), function(x) risk_badge_html(x %||% "LOW"))
  ...
  output$val_ui <- renderUI({
    ...
    if (nrow(pending) == 0)
      return(tags$div(style="text-align:center;padding:60px;color:#6c757d;",
                      bs_icon("check-circle", size="2em"),
                      tags$p(style="margin-top:12px;font-size:14px;", "All cases validated.")))
    ...
    tagList(
      tags$div(style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:8px;",
               tags$div(
                 tags$h5(style="margin:0;font-size:14px;font-weight:700;",
                         tagList(bs_icon("shield-check"), " Officer Validation — NCIC Framework")),
                 tags$p(style="font-size:12px;color:#6c757d;margin:3px 0 0;",
                        paste0(nrow(pending), " pending (L2+) sorted by risk score"))
               ),
               tags$div(style="display:flex;gap:8px;align-items:center;",
                        if (!rv$bulk_running && nchar(rv$learning_flash) > 0)
                          tags$div(style="font-size:11px;background:#EEEDFE;color:#3C3489;border-radius:4px;padding:3px 10px;border:1px solid #AFA9EC;",
                                   "🧠 ", rv$learning_flash) else NULL,
                        downloadButton("dl_val", "⬇ Export",
                                       style="background:#0066cc;color:#fff;border:none;font-weight:600;border-radius:6px;font-size:12px;")
               )
      ),
      ...
      layout_columns(col_widths=c(6,6),
                     lapply(seq_len(nrow(rows)), function(i) {
                       row      <- rows[i,]
                       cid      <- row$case_id %||% ""
                       lvl      <- as.integer(row$ncic_level %||% 0L)
                       nc       <- ncic_color(lvl)
                       risk_lvl <- row$risk_level %||% "LOW"
                       rc       <- if (risk_lvl=="HIGH") "#dc3545" else if (risk_lvl=="MEDIUM") "#fd7e14" else "#198754"
                       sigs     <- strsplit(row$signals %||% "", "\\|")[[1]]
                       cur_note <- rv$notes[[cid]] %||% ""
                       tweet    <- row$tweet_text %||% ""
                       words    <- unique(trimws(unlist(strsplit(gsub("[.,!?;:]", " ", tweet), "\\s+"))))
                       words    <- words[nchar(words) >= 3][seq_len(min(20, sum(nchar(words) >= 3)))]
                       
                       card(style=paste0("border-top:3px solid ", nc, ";margin-bottom:4px;"),
                            ...
                            tags$div(class="val-tweet-box",
                                     style=paste0("border-left:3px solid ", nc, ";font-style:italic;"), tweet),
                            ...
                            tags$div(style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px;padding:0 14px;",
                            ...
                            tags$div(style="padding:0 14px;margin-bottom:8px;",
                                     tags$div(style="font-size:10px;color:#6c757d;margin-bottom:2px;",
                                              paste0("Risk: ", row$risk_score %||% 0, " / 100")),
                                     tags$div(style="height:6px;background:#e9ecef;border-radius:3px;overflow:hidden;",
                                              tags$div(style=paste0("width:", min(100, row$risk_score %||% 0),
                                                                    "%;height:6px;background:", rc, ";border-radius:3px;")))
                            ),
                            
                            if (length(sigs) > 0 && nchar(sigs[1]) > 0)
                              tags$div(style="padding:0 14px;margin-bottom:8px;",
                                       tags$div(style="font-size:10px;color:#7c3aed;font-weight:700;margin-bottom:3px;", "🧠 WHY FLAGGED:"),
                                       tagList(lapply(sigs, function(s) tags$div(class="explain-signal", "🔎 ", s)))
                              ) else NULL,
                            
                            tags$div(
                              style="border:1px solid #e5e7eb;border-radius:8px;padding:12px 14px;margin:8px 14px;background:#fafafa;",
                              tags$div(style="font-size:10px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px;",
                                       "Officer Decision — Structured Reasoning"),
                              
                              tags$div(style="margin-bottom:8px;",
                                       tags$div(style="font-size:11px;font-weight:600;color:#374151;margin-bottom:3px;", "1. Signal word(s):"),
                                       textInput(paste0("sig_words_", cid), NULL, width="100%",
                                                 placeholder="e.g. madoadoa, kabila — comma separated")
                              ),
                              
                              if (length(words) > 0)
                                tags$div(style="margin-bottom:8px;",
                                         tags$div(style="font-size:11px;color:#6c757d;margin-bottom:4px;", "   or tap from the post:"),
                                         tags$div(id=paste0("kw_tokens_", cid), style="line-height:2.2;",
                                                  lapply(words, function(w) {
                                                    tags$span(w,
                                                              style="display:inline-block;padding:2px 9px;margin:2px;border-radius:4px;font-size:12px;cursor:pointer;background:#f3f4f6;border:1.5px solid transparent;",
                                                              onclick=sprintf(
                                                                "var el=this;el.classList.toggle('kw-sel');
                           el.style.background=el.classList.contains('kw-sel')?'#FCEBEB':'#f3f4f6';
                           el.style.color=el.classList.contains('kw-sel')?'#A32D2D':'';
                           el.style.borderColor=el.classList.contains('kw-sel')?'#A32D2D':'transparent';
                           var sel=Array.from(document.querySelectorAll('#kw_tokens_%s .kw-sel')).map(function(e){return e.textContent.trim();});
                           document.getElementById('%s').value=sel.join(', ');
                           Shiny.setInputValue('sig_words_%s',sel.join(', '),{priority:'event'});",
                                                                cid, paste0("sig_words_", cid), cid)
                                                    )
                                                  })
                                         )
                                ) else NULL,
                              ...
                              tags$div(style="margin-bottom:8px;",
                                       ...
                                       selectInput(paste0("sig_denotes_", cid), NULL, width="100%",
                                                   choices=c("Select..."="", "Call to physical violence"="CALL_TO_VIOLENCE",
                                                             "Call to expel/remove community"="CALL_TO_EXPEL",
                                                             "Dehumanisation"="DEHUMANISATION_ANIMAL",
                                                             "Derogatory ethnic slur"="ETHNIC_SLUR",
                                                             "Exclusion rhetoric"="EXCLUSION_RHETORIC",
                                                             "Coded hate speech"="CODED_HATE",
                                                             "Religious hatred"="RELIGIOUS_HATRED", "Other"="OTHER"))
                              ),
                              
                              tags$div(style="margin-bottom:8px;",
                                       tags$div(style="font-size:11px;font-weight:600;color:#374151;margin-bottom:3px;", "3. Language:"),
                                       selectInput(paste0("sig_lang_", cid), NULL, width="100%",
                                                   choices=c("Select..."="", "English"="en", "Swahili"="sw", "Sheng"="sheng",
                                                             "Kikuyu"="kikuyu", "Luo"="luo", "Kalenjin"="kalenjin", "Mixed"="mixed"))
                              ),
                              ...
                              tags$div(style="margin-bottom:8px;",
                                       ...
                                       textAreaInput(paste0("sig_reason_", cid), NULL, width="100%", rows=2,
                                                     placeholder="e.g. 'madoadoa' historically used to call for ethnic violence")
                              ),
                              
                              tags$div(style="margin-bottom:10px;",
                                       tags$div(style="font-size:11px;font-weight:600;color:#374151;margin-bottom:6px;", "5. Context type:"),
                                       tags$div(id=paste0("ctx_area_", cid),
                                                lapply(list(
                                                  list(val="ethnic_incitement",    label="Ethnic incitement",
                                                       color="#E24B4A", desc="Targets group by tribe/religion/region"),
                                                  list(val="political_violence",   label="Political violence",
                                                       color="#BA7517", desc="Violence tied to elections or politics"),
                                                  list(val="historical_reference", label="Historical reference",
                                                       color="#185FA5", desc="References past events"),
                                                  list(val="satire",               label="Satire / humour",
                                                       color="#3B6D11", desc="Intended as joke — reduces keyword weight"),
                                                  list(val="neutral",              label="Neutral / no threat",
                                                       color="#888780", desc="Keywords present but no harmful intent")
                                                ), function(ctx) {
                                                  tags$div(
                                                    style="display:flex;align-items:flex-start;gap:8px;padding:6px 10px;border:0.5px solid #e5e7eb;border-radius:6px;cursor:pointer;margin-bottom:4px;",
                                                    `data-ctx-cid` = cid,
                                                    onclick = sprintf(
                                                      "document.querySelectorAll('[data-ctx-cid=\"%s\"]').forEach(function(e){e.style.borderColor='#e5e7eb';e.style.background='';});this.style.borderColor='%s';this.style.background='%s22';Shiny.setInputValue('ctx_selected_%s','%s',{priority:'event'});",
                                                      cid, ctx$color, ctx$color, cid, ctx$val),
                                                    tags$div(style=paste0("width:10px;height:10px;border-radius:50%;background:",
                                                                          ctx$color, ";flex-shrink:0;margin-top:3px;")),
                                                    tags$div(
                                                      tags$div(style="font-size:12px;font-weight:500;color:#374151;", ctx$label),
                                                      tags$div(style="font-size:11px;color:#6c757d;", ctx$desc)
                                                    )
                                                  )
                                                })
                                       )
                              ...
                              tags$div(style="margin-bottom:8px;",
                                       tags$div(style="font-size:11px;font-weight:600;color:#374151;margin-bottom:3px;", "6. Override NCIC (optional):"),
                                       selectInput(paste0("ncic_ov_", cid), NULL, width="100%",
                                                   choices=c("Agree with AI"="",
                                                             setNames(as.character(0:5), paste0("Override → L", 0:5, " ", NCIC_LEVELS))),
                                                   selected="")
                              ),
                              
                              textAreaInput(paste0("note_", cid), "Officer note", value=cur_note,
                                            placeholder="Justification, context, or instructions…",
                                            rows=2, width="100%"),
                              
                              tags$div(style="border-top:1px solid #e5e7eb;padding-top:10px;margin-top:4px;",
                                       tags$div(style="font-size:10px;font-weight:600;color:#374151;margin-bottom:8px;", "7. Decision:"),
                                       tags$div(style="display:flex;gap:6px;flex-wrap:wrap;",
                                                tags$div(style="display:flex;flex-direction:column;align-items:center;gap:2px;",
                                                         ...
                                                         tags$div(style="font-size:9px;color:#198754;", "AI was correct")),
                                                tags$div(style="display:flex;flex-direction:column;align-items:center;gap:2px;",
