...
#
#  IMPROVEMENTS v2:
#    FIX 1 — Signal yield:    Broader Swahili/Sheng/English keywords
#    FIX 2 — Date filtering:  Only pull messages newer than last run
#    FIX 3 — Kenya filter:    Exclude noise channels (crypto/jobs/property)
...
#
#  NCIC FOCUS:
#    Purely content-based — no geography, no personalities.
#    Captures: hate speech, incitement, dehumanisation,
#              discrimination, undermining national unity.
#
...
LAST_RUN_FILE     = "cache/tg_last_run.json"   # FIX 2: date filter
MESSAGES_LIMIT    = 300     # messages to pull per channel per run
SEARCH_LIMIT      = 10      # channels to discover per keyword
LOOKBACK_HOURS    = 48      # FIX 2: only pull messages from last 48hrs
MIN_MESSAGE_LEN   = 10      # ignore very short messages

os.makedirs("cache", exist_ok=True)

# ── FIX 2: DATE FILTER — load last run timestamp ─────────────────
def load_last_run():
    if os.path.exists(LAST_RUN_FILE):
        with open(LAST_RUN_FILE) as f:
            data = json.load(f)
            return datetime.fromisoformat(data.get("last_run", "2000-01-01T00:00:00")).replace(tzinfo=timezone.utc)
    # First run — go back LOOKBACK_HOURS
...
# Exclude channels that are clearly not radicalisation-related
NOISE_PATTERNS = [
...
# ── FIX 1 & 6: EXPANDED NCIC KEYWORDS (English + Swahili + Sheng) ─
# ================================================================
#  NCIC Signal Keywords — v2
#  Source: NCIC Act Cap 170, NCIC Hate Speech Guidelines,
#          Kenya social media monitoring reports
# ================================================================

# SEARCH KEYWORDS — discover channels on Telegram
SEARCH_KEYWORDS = [
    # English signal terms
    ...
    # Broader discovery — politics channels
    "Kenya politics", "Kenya siasa", "Kenya 2027",
    "uchaguzi Kenya", "Kenya parliament",
]

# ── TIER 3: DIRECT INCITEMENT & DEHUMANISATION (+3 each) ─────────
TIER3 = [
    # English — direct calls to violence
    ...
    # Swahili — direct calls to violence
...
# ── TIER 2: ETHNIC CONTEMPT & DISCRIMINATION (+2 each) ───────────
TIER2 = [
    # Derogatory ethnic labels — Swahili/Sheng
...
# ── TIER 1: DIVISIVE CONTENT & WEAK SIGNALS (+1 each) ────────────
TIER1 = [
    # Ethnic identity in conflict context — Swahili
...
# FIX 3: Minimal but effective Kenya context check
KENYA_CONTEXT = [
...
def ncic_category(triggers):
    if not triggers:
        return "NONE"
    t = triggers.lower()
    if any(k in t for k in ["wauawe","waangamizwe","kill","eliminate","wipe","uprising","jihad","silaha","piga vita"]):
        return "INCITEMENT"
    if any(k in t for k in ["nyoka","mende","panya","cockroach","vermin","wanyama","wadudu","sub-human","not human"]):
        return "DEHUMANISATION"
    if any(k in t for k in ["secede","tujitenganishe","break away","pwani si kenya","jamhuri yetu"]):
        return "SECESSIONISM"
    if any(k in t for k in ["tribal rigging","uchaguzi wa ukabila","stolen by tribe","vote along tribal"]):
        return "ELECTION_INCITEMENT"
    if any(k in t for k in ["dini chafu","religion of evil","adui wa dini","waislamu","wakristo ni maadui"]):
        return "RELIGIOUS_HATRED"
    if any(k in t for k in ["jaluo","madoadoa","kabila chafu","si wakenya","not real kenyans","wahamie"]):
        return "ETHNIC_CONTEMPT"
    if any(k in t for k in ["hate speech","chuki","ubaguzi","discrimination","uchochezi"]):
        return "HATE_SPEECH"
...
async def main():
    ...
    print("=" * 60)
    print("TELEGRAM NCIC SIGNAL PIPELINE v2")
    print(datetime.now().strftime("%d %b %Y %H:%M"))
    print("=" * 60)

    # FIX 2: Load last run timestamp
    since = load_last_run()
    print(f"[date filter] Only pulling messages since: {since.strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    print(f"\n[messages] Pulling messages per channel (since {since.strftime('%Y-%m-%d %H:%M')})...")
    ...
    # FIX 5: Pre-fetch participants for sender resolution (batch)
    sender_cache = {}
    ...
    for chat_id, entity in discovered_channels.items():
        ...
        try:
            # FIX 2: Pull with date offset — stop when messages older than since
            ...
            for msg in messages:
                # FIX 2: Date filter — skip old messages
                ...
                # FIX 5: Sender resolution — use cache, skip slow lookups
            ...
            print(f"  ⚠ Rate limited on {title} — waiting {e.seconds}s")
            await asyncio.sleep(e.seconds)
        except Exception as e:
    ...
    print(f"  Date filtered:     {n_date_skip}  (older than {since.strftime('%Y-%m-%d %H:%M')})")
